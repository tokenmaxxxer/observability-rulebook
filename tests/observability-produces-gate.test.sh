...
run_tool deny Write "$DEFAULT_PATH" "$(python3 -c 'import json,sys; print(json.dumps({"file_path":sys.argv[1],"content":sys.argv[2]}))' "$DEFAULT_PATH" "$MISSING_CONTENT")" "" 'CLAUDE_PLUGIN_ROOT_CORE=/nonexistent-core-path-xyz' missing-core-denies

# --- h. Bash-write coverage: a Bash command targeting the record path must deny ---
run_tool deny Bash "$DEFAULT_PATH" "$(python3 -c 'import json,sys; print(json.dumps({"command": "cat > " + sys.argv[1]}))' "$DEFAULT_PATH")" "" "" bash-write-denies
run_tool allow Bash "$DEFAULT_PATH" '{"command":"ls -la"}' "" "" bash-unrelated-allows
